...
class LegalRegulatoryAgent:
    def __init__(self):
        self.name = "Legal & Regulatory"
        self.agent_key = "legal_regulatory"
        
    def get_system_prompt(self) -> str:
        """
        Strict JSON-only system prompt for Legal & Regulatory compliance analysis
        """
...
You are a Legal & Regulatory compliance agent. Analyze content for legal risks and regulatory violations.

SCORING (0.0-1.0, where 1.0 = legally compliant):
- ≥0.8: "none" (no legal risks)
- 0.6-0.7: "low-risk" (minor legal concerns)
- 0.3-0.5: "medium-risk" (regulatory issues)
- <0.3: "high-risk" (major legal violations)

VIOLATION TYPES (max 3, only if significant):
["regulatory_violation", "legal_risk", "unauthorized_practice"]

REQUIREMENTS:
- Keep reasoning to 1 concise sentence
- Keep findings to 1 sentence each (max 2)
- Keep recommendations to 1 sentence each (max 2)
- Only flag ACTUAL legal/regulatory violations, not theoretical risks
- Simple conversations should score 0.9+ with no violations
...
RETURN ONLY THIS JSON FORMAT:
{
    ...
    "reasoning": "One sentence explaining the legal assessment",
...
Analyze the above content thoroughly for legal risks, regulatory violations, and compliance issues. Return only the JSON response as specified."""
    ...
    def parse_agent_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Legal Regulatory Agent analysis complete. Score: {result['score']}, Flag: {result['flag_type']}")
            
            return result
            
        except (json.JSONDecodeError, ValueError) as e:
            logger.error(f"Failed to parse Legal Regulatory Agent response: {str(e)}")
