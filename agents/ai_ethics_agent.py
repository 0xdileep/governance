...
class AIEthicsAgent:
    def __init__(self):
        self.name = "AI Risk & Ethics"
        self.agent_key = "ai_risk_ethics"
        
    def get_system_prompt(self) -> str:
        """
        Strict JSON-only system prompt for AI Risk & Ethics compliance analysis
        """
...
You are an AI Risk & Ethics compliance agent. Analyze content for bias and ethical issues.

SCORING (0.0-1.0, where 1.0 = ethical):
- ≥0.8: "none" (no ethical issues)
- 0.6-0.7: "low-risk" (minor concerns)
- 0.3-0.5: "medium-risk" (bias detected)
- <0.3: "high-risk" (clear discrimination)

VIOLATION TYPES (max 3, only if significant):
["potential_bias", "discrimination_risk", "ethical_concern"]

REQUIREMENTS:
- Keep reasoning to 1 concise sentence
- Keep findings to 1 sentence each (max 2)
- Keep recommendations to 1 sentence each (max 2)
- Only flag ACTUAL bias/discrimination, not theoretical concerns
...
RETURN ONLY THIS JSON FORMAT:
{
    ...
    "reasoning": "One sentence explaining the ethical assessment",
...
Analyze the above content thoroughly for AI ethics, bias, and compliance issues. Return only the JSON response as specified."""
    ...
    def parse_agent_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"AI Ethics Agent analysis complete. Score: {result['score']}, Flag: {result['flag_type']}")
            
            return result
            
        except (json.JSONDecodeError, ValueError) as e:
            logger.error(f"Failed to parse AI Ethics Agent response: {str(e)}")
