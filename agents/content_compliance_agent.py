...
class ContentComplianceAgent:
    def __init__(self):
        self.name = "Content & Platform Compliance"
        self.agent_key = "content_platform"
        
    def get_system_prompt(self) -> str:
        """
        Strict JSON-only system prompt for Content & Platform compliance analysis
        """
...
You are a Content & Platform compliance agent. Analyze content for harmful content and policy violations.

SCORING (0.0-1.0, where 1.0 = safe content):
- ≥0.8: "none" (safe content)
- 0.6-0.7: "low-risk" (minor concerns)
- 0.3-0.5: "medium-risk" (policy violations)
- <0.3: "high-risk" (harmful content)

VIOLATION TYPES (max 3, only if significant):
["harmful_content", "misinformation", "policy_violation"]

REQUIREMENTS:
- Keep reasoning to 1 concise sentence
- Keep findings to 1 sentence each (max 2)
- Keep recommendations to 1 sentence each (max 2)
- Only flag ACTUAL harmful content or clear policy violations
...
RETURN ONLY THIS JSON FORMAT:
{
    ...
    "reasoning": "One sentence explaining the content assessment",
...
Analyze the above content thoroughly for harmful content, misinformation, and platform compliance issues. Return only the JSON response as specified."""
    ...
    def parse_agent_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Content Compliance Agent analysis complete. Score: {result['score']}, Flag: {result['flag_type']}")
            
            return result
            
        except (json.JSONDecodeError, ValueError) as e:
            logger.error(f"Failed to parse Content Compliance Agent response: {str(e)}")
